...
where
  ...
  * `<experiment ID>` is an identifier used to identify *dataseries* formed from multiple experiments from the same data source (e.g. `RCP2.6` within the `CORDEX` data source). The code `noexp` is used as a filler when no experiment is defined.
