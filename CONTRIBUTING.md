...
* KAPy uses the [black](https://black.readthedocs.io/en/stable/) code formatter together with ruff for linting. These are both managed via pre-commit hooks, and can be installed by running `pre-commit install` in the root directory of the repository.
