...
def mergeCSVs(outFile, inFiles):
    ...
    for f in inFiles:
        ...
        if firstFile:
            column_order=df.columns  #Fix column order to match the first file. Avoid problems with switching
        df.to_csv(outFile,index=False,columns=column_order,mode="a",header=firstFile)
        firstFile=False
