...
def getWorkflow(config):
    ...
    for thisKey, thisInp in config["inputs"].items():
        ...
        if len(inpTbl) == 1:
            ...
            pvTbl = inpTbl
            pvTbl["pvFname"] = (
                f"{thisInp['dataset_code']}_{thisInp['variable_code']}_{thisInp['gridCode']}_noexp_noensid.{fileExtn}"
            )
        ...
        # however, that in some cases we may want to use the ensemble ID definitions anyway
            ...
            pvTbl = inpTbl
            pvTbl["pvFname"] = (
                f"{thisInp['dataset_code']}_{thisInp['variable_code']}_{thisInp['gridCode']}_"
                + inpTbl["exptID"]
                + "_"
                + inpTbl["ensMemberID"]
                + f".{fileExtn}"
            ...
            # Else, handle the more complex case where we have defined a common experiment
                ...
                for thisExpt in otherExptList:
                    ...
                    theseExptFiles["pvFname"] = (
                        f"{thisInp['dataset_code']}_{thisInp['variable_code']}_{thisInp['gridCode']}"
