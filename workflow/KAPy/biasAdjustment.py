...
def biasAdjust(
    ...
    **kwargs,
    ...
    regrdWtsFname = tempfile.NamedTemporaryFile(
        dir=tempDir, delete=False, prefix="regrdWts_", suffix=".nc"
    ).name
    regrdr = xe.Regridder(
        ds_in=from_this_grid,
        ds_out=to_this_grid,
        method="nearest_s2d",
    ...
    regridded_filename = tempfile.NamedTemporaryFile(
        dir=tempDir, delete=False, prefix="regridded_", suffix=".nc"
    ).name
    regridded = regrdr(rechunked, output_chunks=(-1, -1), keep_attrs=True)
    chunkThisWay = helpers.align_chunking(regridded)
    regridded.to_netcdf(
        regridded_filename, encoding={regridded.name: {"chunksizes": chunkThisWay}}
    )
    ...
    combDS2 = xr.Dataset(
        {"target": target.unify_chunks(), "ref": reference_common.unify_chunks()}
    )
    ...
    def biasAdjustThisChunk(
        ...
        tgTP = tgTP.convert_calendar(
            rfTP.time.dt.calendar, use_cftime=True, align_on="year"
        )
        ...
        if method == "xsdba-eqm":
            ...
            raise ValueError(
                '"custom" bias adjustment functions are currently not implemented'
            )
